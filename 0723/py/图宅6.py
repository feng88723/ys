...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        html = r.text
        imgs += self.getImgs(html)
        
        # 解析分页器，获取所有页面链接
        pager_match = re.search(r'<div id="pager".*?</div>', html, re.S)
        if pager_match:
            pager_html = pager_match.group(0)
            # 提取所有包含 at= 的链接（如 ?at=2）
            page_links = re.findall(r'href=["\']([^"\']+?at=\d+)[^"\']*["\']', pager_html)
            # 去重并排序（按页码）
            unique_links = set()
            for link in page_links:
                ...
                if not link or link == '#':
                    continue
                # 使用站点根URL拼接绝对路径（因为链接是绝对路径）
                if link.startswith('/'):
                    full_url = self.siteUrl + link
                else:
                    full_url = urljoin(base_url, link)
                unique_links.add(full_url)
            
            # 限制最多请求30页，避免过多请求
            page_urls = list(unique_links)
            # 可选：按页码排序（提取 at= 数字）
            page_urls.sort(key=lambda u: int(re.search(r'[?&]at=(\d+)', u).group(1)) if re.search(r'[?&]at=(\d+)', u) else 0)
            for page_url in page_urls:
                # 跳过第一页（如果第一页也在列表中，但第一页通常没有参数，所以不会出现）
        ...
        return {"parse": 0, "url": "pics://" + "&&".join(unique_imgs) if unique_imgs else "", "header": ""}

    def getImgs(self, h):
        # 从 file-detail 区域提取图片（优先 data-src，其次 src）
        b = re.search(r'<div class="file-detail">(.*?)<div id="pager"', h, re.S) or \
            re.search(r'<div class="file-detail">(.*?)<div class="related-file">', h, re.S)
        t = b.group(1) if b else h
        # 提取所有图片链接，同时匹配 data-src 和 src
        imgs = re.findall(r'<img[^>]+(?:data-src|src)=["\']([^"\']+)["\']', t)
        return [("https:" + x if x.startswith("//") else x) for x in imgs]