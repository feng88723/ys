...
_PAGE_SIZE = 8
_WORKERS = 3  # 降低并发线程数，防止移动端 APP 闪退
_TIMEOUT = 3
...
class Spider(Spider):
    ...
    def _fetch_mp4(self, url, depth=0):
        # 限制最大递归深度，防止无限重定向导致堆栈溢出崩溃
        if depth > 2:
            return "", 0
        try:
            with self._s.get(url, timeout=_TIMEOUT, allow_redirects=True, stream=True) as r:
            ...
            # 读取 body 尝试解析
            with self._s.get(url, timeout=_TIMEOUT, allow_redirects=False) as r2:
                txt = r2.text.strip()
            ...
            if txt.startswith("http"):
                if self._is_video_url(txt):
                    return txt, 0
                return self._fetch_mp4(txt, depth + 1)
    ...
    def homeVideoContent(self):
        # 首页不预加载视频，避免首页一次性并发请求 50+ 个接口导致崩溃
        return []
